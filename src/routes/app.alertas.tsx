    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          {
            l: "Críticos",
            v: mockAlerts.filter((a) => a.severity === "critical").length,
            t: "destructive" as const,
          },
          {
            l: "Alta",
            v: mockAlerts.filter((a) => a.severity === "high").length,
            t: "warning" as const,
          },
          {
            l: "Pendentes",
            v: mockAlerts.filter((a) => !a.resolved).length,
            t: "primary" as const,
          },
          {
            l: "Resolvidos",
      ...
      <Panel bodyClassName="p-0">
        <ul className="divide-y divide-border">
          {mockAlerts.map((a) => (
            ...
            >
              <div
                className={`h-9 w-9 rounded-lg grid place-items-center shrink-0 ${a.resolved ? "bg-success/10 text-success" : "bg-destructive/10 text-destructive"}`}
              >
                {a.resolved ? (
